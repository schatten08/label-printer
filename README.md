...
1.  Скачайте проект целиком (Архив ZIP или `git clone`), распакуйте и запустите `Run_Printer.bat`.
    ...
    *   Дальше скрипт полностью автоматически:
        ...
        3. Проверит, установлен ли **драйвер принтера Brother** — и если нет, скачает и запустит официальный установщик "Software/Document Installer" (b-PAC сам по себе не умеет печатать на физическом принтере; пройдите его короткий мастер установки).
    ...
    *   ⚠️ Если после нажатия "Печать" статус показывает "Напечатано" (или программа вовсе зависает), но наклейка не выходит — почти всегда это значит, что не установлен полный пакет ПО Brother, а не только b-PAC. Установите **"Software/Document Installer (Recommended)"** (он ставит и драйвер принтера, и P-touch Editor вместе — одного драйвера может быть недостаточно) с [официального сайта Brother](https://support.brother.com/g/b/downloadtop.aspx?c=us&lang=en&prod=lpql810weus).
