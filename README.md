...
3. Запустите скрипт через файл **`ЭТИКЕТКИ_ИНТЕРФЕЙС.bat`** (он использует `pythonw` для скрытия консольного окна).
...
- `ЭТИКЕТКИ_ИНТЕРФЕЙС.bat` — Файл для быстрого запуска UI.
